...
bool Node::IsDivided() const // 분할되었는지 확인
{
    // 자손노드 4개 중 하나라도 null이 아니면 분할됐다고 판단
    return topLeft != nullptr; // !!topLeft
}
...
Node::NodeIndex Node::TestRegion(const Bounds& targetBounds) const
{
    ...
    // 반환 받은 배열의 요소 개수가 1개라면 4분면 중 1개 영역에 완전 포함
    if (quads.size() == 1) {
                return quads[0];
    }
...
{
    // 겹치는 영역을포함한 배열
    std::vector<Node::NodeIndex> quads;
    ...
    // 왼쪽 영역에 완전히 포함되는지 확인
	bool left = targetBounds.GetXMax() <= x && targetBounds.GetXMax() <= centerX;
    
    // 오른쪽 영역에 완전히 포함되는지 확인
    bool right = targetBounds.GetX() >= centerX && targetBounds.GetXMax() < x + bounds.GetXMax();

    // 위쪽 영역에 완전히 포함되는지 확인
	bool top = targetBounds.GetY() <= y && targetBounds.GetYMax() <= centerY;

    // 아래쪽 영역에 완전히 포함되는지 확인
	bool bottom = targetBounds.GetY() >= centerY && targetBounds.GetYMax() < y + bounds.GetYMax();
